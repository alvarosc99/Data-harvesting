...
```{r}
client_ID <- scan("C:/Users/alvar/Desktop/client_id.txt", what = character()) # Enter the correct path where the client_id is located. 
```


```{r}
client_secret <- scan("C:/Users/alvar/Desktop/client_secret.txt", what = character()) # Enter the correct path where the client_secret is located.
```
...
```{r}
dance <- top50full %>% 
  ggplot(aes(x=Position, y=danceability), text = paste(name...10)) +
  geom_point() +
  ylim(c(0, 1))
...
```{r}
acoustic <- top50full %>% 
  ggplot(aes(x=Position, y=acousticness), text = paste(name...10)) +
  geom_point() +
  ylim(c(0, 1))
...
```{r}
speech <- top50full %>% 
  ggplot(aes(x=Position, y=speechiness), text = paste(name...10)) +
  geom_point() +
  ylim(c(0, 0.5))
...
```{r}
viz4 <- ggplot(tracks2, aes(x=danceability, fill=playlist,
                    text = paste(playlist)))+
...
```{r}
viz4 <- ggplot(tracks2, aes(x=valence, fill=playlist,
                    text = paste(playlist)))+
  geom_density(alpha=0.7, color=NA)+
  scale_fill_manual(values=c("blue", "orange"))+
  ...
  xlim(c(0.25,1)) +
  theme_minimal()+
  ggtitle("Distribution of Positiveness")

ggplotly(viz4, tooltip=c("text"))
```

```{r}
viz4 <- ggplot(tracks2, aes(x=speechiness, fill=playlist,
                    text = paste(playlist)))+
  geom_density(alpha=0.7, color=NA)+
  scale_fill_manual(values=c("blue", "orange"))+
  labs(x="Danceability", y="Density") +
...
```{r, fig.height=5}
comparison %>% 
  ggplot(aes(y = Track_match, x=reference)) +
...
#Intentar adaptar al formato req inicial, para estandarizar
...
features <- function(x) {
  ...
  req <- 
    ...
    req_perform() %>% 
    resp_body_json(simplifyVector = T) %>% 
    as_tibble() ## Adaptar al formato req inicial, para estandarizar
