...
As a final step, we may combine the utilities of the custom functions, the historical All Out playlist, and the analysis of the musical features to analysis the diferences in styles for the music for the different decades. 
...
featuresAO_plot <- ggplot(AO_Full) +
  aes(x = danceability, y = energy, colour = playlist) +
  geom_point() +
  scale_size_area(max_size = 20)+
  theme_minimal()
...
```{r}
viz5 <- ggplot(AO_Full, aes(x=danceability, fill=playlist,
                    text = paste(playlist)))+
  geom_density(alpha=0.7, color=NA)+
